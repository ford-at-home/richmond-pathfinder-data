...
| ------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------ | ----------------------------- | ---------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------- |
| `/`                                         | `/`                                                                                       | `src/routes/index.tsx`                                 | report blurbs, lock pin                    | none                          | analysis pin in footer             | migrated       | Demonstrator not linked (U1). Reports lead.                                                           |
| `/report/ai-exposure-and-employment-change` | `/research/ai-exposure-and-employment-change` plus redirect from `/report/$slug`          | `data/source/reports/…md`, `src/lib/reports.ts`        | markdown pin, figure placement             | six live figures              | hash-pinned markdown, never edited | migrated       | Byte-identical source file                                                                            |
| `/report/transition-capacity`               | `/research/transition-capacity` plus `/transition-capacity` quoting the published summary | same                                                   | markdown pin, pathways figure              | DestinationScarcity           | same                               | migrated       | Six-stage calculator remains empty                                                                    |
| `/report/technical-appendix`                | `/research/technical-appendix`                                                            | same                                                   | markdown + generated tables                | none                          | same                               | migrated       |                                                                                                       |
...
| _(no Cytoscape map)_                        | `/transition-map`                                                                         | `pathways_reachable.csv`, `richmond_exposure_2025.csv` | table of screened pairs                    | filter by origin              | codebook + findings                | migrated       | Source has no network graph. Table is the source behavior. Cytoscape slot labelled not in the source. |
| _(no MapLibre map)_                         | `/richmond-region`                                                                        | `qcew_fixed_geography.csv`, codebook geography         | MSA definition, QCEW industry series       | none                          | MSA 40060; county change 2023/24   | migrated       | No per-locality occupation table. Localities array empty.                                             |
| _(methodology.md in vendor)_                | `/methodology`                                                                            | `data/source/docs/methodology.md` and `robustness.md`  | join, metrics, limits                      | none                          | vendor docs                        | migrated       | Documents rendered, not paraphrased                                                                   |
| n/a                                         | `/research`                                                                               | index of the three reports                             | REPORTS blurbs                             | search/filter                 |                                    | migrated       | Placeholder stories removed                                                                           |
